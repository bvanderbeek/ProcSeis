function [delays,tpick,ibad,CCF,itol,S,seis] = ICCS_OG(tstart,fs,seis,ttaper,wstart,wlength,ccf_min,PickParams,varargin)
...
while (tol > tol_max) && (niter < niter_max)
    ...
    for ii = 1:ntrace
        ...
        [r,lags] = xcorr(si.*w,S.*w); % Window both traces...
        %[r,lags] = xcorr(si,S.*w); % Or just window the stacked trace? This sometimes results in cycle skipping.
        lags     = lags./fs;
        r        = r.*((lags >= -wlength/2) & (lags <= wlength/2)); % ...but only selecting maximum correlation in window
        keep     = find((r == max(r)),1,'first'); % A crude choice for delay
        DT(ii)   = lags(keep);
        CCF(ii)  = max(r); 
