...
% MODIFIED VERSION THAT ROTATES SPECFEM SEISMOGRAMS TO ACCOUNT FOR EARTH
% CURVATURE
...
if tf_rotate && (data.nchan > 1)
    if data.nchan == 3
        disp('ADJUSTING FOR EARTH CURVATURE!');
    ...
    for ii = 1:data.ntrace
        ...
        elseif data.nchan == 3
            ...
            % Extract 3-component trace in X-Y-Z
            F = [squeeze(data.seis(ii,1,:))'; squeeze(data.seis(ii,2,:))'; squeeze(data.seis(ii,3,:))'];
            % Because we used SPECFEM cartesian, we need to rotate the
            % seismograms to East-North-Radial components!!!.
            [~,jsta] = ismember(data.station(ii), station.id);
            slon = station.longitude(jsta);
            slat = station.latitude(jsta);
            % X = East, Y = North, R = Z.
            % CW rotation around x-axis for positive latitude = negative rotation angle
            % CCW rotation around y-axis for positive longitude = positive rotation angle
            % We are rotating East-North-Radial into X-Y-Z, hence the
            % transpose.
            F = transpose(roty(slon)*rotx(-slat))*F;
            % Rotate by +baz since baz is measured CW from N (rotz angle is CCW
            % from +x). Channel 1 is the transverse and cshannel 2 is radial.
            F = rotz(R.orientation(R.ista(ii)))*F;
